...
if [ "$JAVA" == "" ]; then
  warning "Some apps can bundle vulnerable library in their own compiled package, so 'java' might not be installed but one such apps could still be vulnerable and be executed."
fi
echo
warning "This whole script is not 100% proof the system is vulnerable"
echo
